...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc func printQuote1(_ sender: Any?) {
        // ダイアログ表示
        let vc = MOS_201A_ViewController()
        vc.MOS_201A_FontDownload()
    }
    
    // アイコン2番目選択時
    @objc func printQuote2(_ sender: Any?) {
        // ダイアログ表示
        let vc = MOS_202A_ViewController()
        vc.MOS_202A_FontDownload()
    }
    
    // アイコン3番目選択時
    @objc func printQuote3(_ sender: Any?) {
        // ダイアログ表示
        let vc = MOS_203A_ViewController()
        vc.MOS_203A_FontDownload()
    }
